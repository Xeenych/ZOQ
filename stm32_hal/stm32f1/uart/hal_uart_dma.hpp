...
public:
	hal_uart_dma(UART_HandleTypeDef *h, uint8_t *rxbuffer, size_t rxbuffer_size,
			uint8_t *txbuffer, size_t txbuffer_size);
...
inline hal_uart_dma::hal_uart_dma(UART_HandleTypeDef *h, uint8_t *_rxbuffer,
		size_t _rxbuffer_size, uint8_t *_txbuffer, size_t _txbuffer_size) :
		handle(h), rxbuffer(_rxbuffer), rxbuffer_size(_rxbuffer_size), txbuffer(
				_txbuffer), txbuffer_size(_txbuffer_size), read_ptr(_rxbuffer) {
	switch ((uint32_t) h->Instance) {
	case USART1_BASE:
		hal_uart_dma::uart1 = this;
		break;
	case USART2_BASE:
		hal_uart_dma::uart2 = this;
		break;
	case USART3_BASE:
		hal_uart_dma::uart3 = this;
		break;
	default:
		break;
	}
...
inline size_t hal_uart_dma::transmit(uint8_t *pData, uint16_t Size,
		uint32_t Timeout) {
...
inline size_t hal_uart_dma::bytes_available() const {
	uint8_t *write_ptr = rxbuffer + rxbuffer_size
			- __HAL_DMA_GET_COUNTER(handle->hdmarx);
...
inline size_t hal_uart_dma::receive(uint8_t *pData, uint16_t Size,
		uint32_t Timeout) {
	uint32_t tickstart = HAL_GetTick();
	...
	while (bytes_copied < Size) {
		...
		size_t bytes_available =
				(write_ptr >= read_ptr) ?
						write_ptr - read_ptr :
						rxbuffer + rxbuffer_size - read_ptr;

		size_t bytes_left_to_read = Size - bytes_copied;
		size_t bytes_to_copy =
				(bytes_available > bytes_left_to_read) ?
						bytes_left_to_read : bytes_available;
