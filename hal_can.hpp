...
inline bool hal_can::readMessage(CAN_RxMsg &msgout) {
	HAL_StatusTypeDef result0 = HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0,
			&msgout.header, msgout.data);
	if (result0 == HAL_OK)
		return true;

	HAL_StatusTypeDef result1 = HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO1,
			&msgout.header, msgout.data);
	if (result1 == HAL_OK)
		return true;
